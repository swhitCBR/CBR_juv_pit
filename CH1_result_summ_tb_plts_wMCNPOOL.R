...
ggplot() +
  geom_errorbar(data=CH1_wtAVG_tb_l_wt2,
                aes(x=as.numeric(defin_det_yr),#factor(defin_det_yr),
                    y=S1,
                    ymin=S1-s1SE*1.96,
                    ymax=S1+s1SE*1.96),
                width=0,
  ...
  geom_point(data=CH1_wtAVG_tb_l_wt2,
             aes(x=as.numeric(defin_det_yr),#defin_det_yr,#factor(defin_det_yr),
                 size=binsize,
                 y=S1),
             position = position_dodge(width=0.5),
             shape=21,fill="white") +
...
# VARIANT WITH H and W combine and
ggplot() +
  geom_errorbar(data=CH1_wtAVG_tb_l_wt2,
                aes(x=as.numeric(defin_det_yr),#factor(defin_det_yr),
                    y=S1,
  ...
  scale_alpha_manual(values=c(0.35,0.85))+#(values=c(2,3,4,5,6,7)) +
  scale_x_continuous(breaks=seq(2000,2025,5)) + 
