...
MCN_BON_2perDF %>% group_by(dat_grp,esutype,reartype,defin_det_yr,bin) %>%
  summarize(n_bins=length(unique(binID)))
...
# applying admissibility criteria
...
sub <- MCN_BON_2perDF_filtbinsub <- MCN_BON_2perDF_filt %>% 
  filter(dat_grp=="mcn_det" & esutype=="SR_Ch1", reartype=="H",
...
tmp$category[!is.na(tmp$s1) & is.finite(tmp$s1) &  tmp$s1>1.1]="Inadmissible S>1.1"
...
tmp12summ$p1_lw=(tmp12summ$p1-unlist(tmp12summ$p1SE))
tmp12summ$p1_hi=(tmp12summ$p1+unlist(tmp12summ$p1SE))
