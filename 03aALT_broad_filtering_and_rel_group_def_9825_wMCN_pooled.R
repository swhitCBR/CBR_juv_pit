...
sub_mcn_pooled_obs<- obs_comb[ obs_comb$obssite %in% c("MCJ"),]
...
obs_rel_grps <- bind_rows(
  ...
  data.frame(dat_grp="mcn_pooled",subset(obs_comb,tagid %in% sub_mcn_pooled_obs$tagid))
  )
...
tmpB <- obs_rel_grps[#obs_rel_grps$stage=="J" & 
  obs_rel_grps$dat_grp %in% c("mcn_det","mcn_pooled") &  (obs_rel_grps$obssite %in% c("MCJ")),]
tmp <- rbind(tmpA,tmpB)
...
tagDF_rel_grps <- rbind(
  ...
  data.frame(dat_grp="mcn_pooled",tags_comb[tags_comb$tagid %in% sub_mcn_pooled_obs$tagid,]))
...
table(obs_rel_grps2$dat_grp,obs_rel_grps2$unassagn)
...
saveRDS(obs_rel_grps,"comp_files/obs_rel_grps_wMCNPOOL.rds")
saveRDS(defin_detDF,"comp_files/defin_detDF_wMCNPOOL.rds")
# before removing negative values relative to the definitive detection
saveRDS(obs_rel_grps2,"comp_files/obs_rel_grps_2_9825_wMCNPOOL.rds")
