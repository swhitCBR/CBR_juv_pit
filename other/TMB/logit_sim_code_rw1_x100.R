...
saveRDS(my_ls,"mylogitsim_x100.rds")


mylogitsim15<- rbind(
  do.call(rbind,readRDS("mylogitsim_x100.rds")))
...
ggplot(data=mylogitsim15,aes(x=P2,y=S1,color=Lambda)) + 
  ...
  geom_point(x=mean(mylogitsim15$P2,na.rm=T),
             y=mean(mylogitsim15$S1,na.rm=T),shape=21,color="yellow") 



ps_samp_dist_plt <- ggplot(data=mylogitsim15,aes(x=P2,y=S1,color=Lambda)) + 
  geom_density_2d_filled(bins=40) + 
...
lp_samp_dist_plt <- ggplot(data=mylogitsim15,aes(x=Lambda,y=P2)) +#,color=Lambda)) + 
  geom_density_2d_filled(bins=40) + 
  # geom_point(shape=16,alpha=0.1) +
  ...
  geom_point(x=mean(mylogitsim15$Lambda,na.rm=T),
             y=mean(mylogitsim15$P2,na.rm=T),shape=21,color="yellow") +
  theme(legend.position = "none")
...
grid.arrange(ps_samp_dist_plt,
             ls_samp_dist_plt,
             lp_samp_dist_plt,
             ggplot(data=mylogitsim15,aes(x=nll)) + 
