...
mysim15<- rbind(
  do.call(rbind,readRDS("mysim1_x100.rds")))

mysim15$P2 <- plogis(mysim15$P2)
mysim15$S1 <- exp(mysim15$S1)
mysim15$Lambda <- plogis(mysim15$Lambda )

library(ggplot2)
ggplot(data=mysim15,aes(x=P2,y=S1,color=Lambda)) + 
  geom_density_2d_filled() + 
  geom_point(shape=16,alpha=0.1) +
  geom_vline(xintercept = P2)+ geom_hline(yintercept = S1) + 
  # geom_vline(xintercept = P2,color="gray")+ geom_hline(yintercept = S1) + 
  geom_point(x=median(mysim15$P2,na.rm=T),
             y=median(mysim15$S1,na.rm=T),shape=21,color="red") +
  geom_point(x=mean(mysim15$P2,na.rm=T),
             y=mean(mysim15$S1,na.rm=T),shape=21,color="blue") 



ps_samp_dist_plt <- ggplot(data=mysim15,aes(x=P2,y=S1,color=Lambda)) + 
  ...
  geom_point(x=median(mysim15$P2,na.rm=T),
             y=median(mysim15$S1,na.rm=T),shape=21,color="red") +
  geom_point(x=mean(mysim15$P2,na.rm=T),
             y=mean(mysim15$S1,na.rm=T),shape=21,color="yellow")  +
  theme(legend.position = "none")

ls_samp_dist_plt <- ggplot(data=mysim15,aes(x=Lambda,y=S1,color=Lambda)) + 
  geom_density_2d_filled(bins=40) + 
  geom_point(shape=16,alpha=0.1,size=0.1) +
  geom_vline(xintercept = Lambda)+ geom_hline(yintercept = S1) + 
  geom_point(x=median(mysim15$Lambda,na.rm=T),
             y=median(mysim15$S1,na.rm=T),shape=21,color="red") +
  geom_point(x=mean(mysim15$Lambda,na.rm=T),
             y=mean(mysim15$S1,na.rm=T),shape=21,color="yellow")  +
  theme(legend.position = "none")


lp_samp_dist_plt <- ggplot(data=mysim15,aes(x=Lambda,y=P2)) +#,color=Lambda)) + 
  geom_density_2d_filled(bins=40) + 
  geom_point(shape=16,alpha=0.1,size=0.1) +
  geom_vline(xintercept = Lambda)+
  geom_hline(yintercept = P2) + 
  geom_point(x=median(mysim15$Lambda,na.rm=T),
             y=median(mysim15$P2,na.rm=T),shape=21,color="red") +
  geom_point(x=mean(mysim15$Lambda,na.rm=T),
             y=mean(mysim15$P2,na.rm=T),shape=21,color="yellow") +
  theme(legend.position = "none")


library(gridExtra)
grid.arrange(ps_samp_dist_plt,
             ls_samp_dist_plt,
             lp_samp_dist_plt,
...
mean(mysim15$nll > nll_obs_numMLE,na.rm = T)
mean(mysim15$nll[1:1000] > nll_obs_numMLE,na.rm = T)

table(mysim15$nll[!is.na(mysim15$nll)] > nll_obs_numMLE)
